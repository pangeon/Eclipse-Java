...
public class StreamNumRefactorBoxed {
    private static final int DIVIDER = 10;

    public static void main(String[] args) {
        //Stream<Integer> numStream = Stream.iterate(0, x -> x + 1);
        List<Integer> squareList = IntStream.range(0,100)
                .map(StreamNumRefactorBoxed::square).boxed()
                .collect(Collectors.toList());
        Set<Integer> biggerThanFive = squareList.stream()
                .filter(StreamNumRefactorBoxed::isDividedBy)
                .map(StreamNumRefactorBoxed::divide)
